...
# Manage Project Page
#
# $RCSfile: project_manage_reqfunctionality_page.php,v $ $Revision: 1.3 $
# ------------------------------------
...
if( !empty( $project_details ) ) {
	...
	# ----------------------------------------------------------------------------------
	if( $project_manager ) {
		print"<form method=post action='project_add_reqfunctionality_action.php'>". NEWLINE;
		...
		print"<tr>". NEWLINE;
		print"<td class=form-header-l>".lang_get('add_req_functionality')."</td>". NEWLINE;
		print"</tr>". NEWLINE;
		print"<tr>". NEWLINE;
		print"<td class='form-lbl-c'>". lang_get('req_functionality') ." <span class='required'>*</span>". NEWLINE;
		print"<input type=text size=60 maxlength=50 name='req_functionality_required' value='".session_validate_form_get_field( 'req_functionality_required' )."'>". NEWLINE;
	...
	if( $rows_req_functionality ) {
		...
		foreach($rows_req_functionality as $row_req_functionality) {
			$id		= $row_req_functionality[REQ_FUNCT_ID];
			$name 	= $row_req_functionality[REQ_FUNCT_NAME];
			...
			if( $project_manager ) {
				print"<td><a href='project_edit_reqfunctionality_page.php?req_functionality_id=$id'>".lang_get("edit")."</a></td>". NEWLINE;
				print"<td>". NEWLINE;
