...
# Manage Project Page
#
# $RCSfile: project_manage_reqdoctype_page.php,v $ $Revision: 1.3 $
# ------------------------------------
...
if( !empty( $project_details ) ) {
	...
	if( $rows_req_doc_types ) {
		...
		foreach($rows_req_doc_types as $row_req_doc_type) {
			$id			= $row_req_doc_type[REQ_DOC_TYPE_ID];
			$name 		= $row_req_doc_type[REQ_DOC_TYPE_NAME];
			$root_doc	= $row_req_doc_type[REQ_DOC_TYPE_ROOT_DOC];
			...
			if( $project_manager ) {
				print"<td>$root_doc</td>". NEWLINE;
				print"<td><a href='project_edit_reqdoctype_page.php?req_doc_type_id=$id'>".lang_get("edit")."</a></td>". NEWLINE;
				print"<td>". NEWLINE;
		...
		html_no_records_found_message( lang_get('no_reqdoctype') );
	}
}
...
# ------------------------------------
# $Log: project_manage_reqdoctype_page.php,v $
